...
\item[1\coe{}\sep1\pik; ?]~
  ...
  \item[1\SA] 12-14 FP \bal
    \bdsc
    \item[2\tre] \conv{Relaistransfer} \ra 2\kar (\ra \ref{1sarebid}, S.~\pageref{1sarebid})
    \item[2\kar] 4er-\pi, 5\pl\kar, zum Spielen
    \item[2\SA] \conv{Relaistransfer} \ra 3\tre (\ra \ref{1sarebid}, S.~\pageref{1sarebid})
    \item[3\tre] \nat, forcing
      \bdsc
...
3\SA && zum Spielen\\
4\tre && \conv{\Index{Gerber}}-Assfrage (0/4, 1, 2, 3; \ra~\ref{gerber}, S.~\pageref{gerber})\\
...
Die 2\pik-Er"offnung zeigt ein \Index{Weak Two} in \pi, d.h. eine 6er-Farbe mit
6-10 FP
...
Teile des Systems abgewandelt werden m"ussen (soweit nicht im Rahmen der
Konvention
erw"ahnt).
...
Ebenfalls erhalten bleibt die Konvention \conv{Sandwich Notrump}
(siehe Seite~\pageref{sandwichnt}).  Folglich gilt \conv{Raptor} nur als
direkter
...
\begin{compactitem}
\item Gebote, die zum Spielen oder Ausbessern gedacht sein k"onnen, sind dies
auch.
...
der
rechte Gegner nicht spricht (nach 1\kar{}- bzw. 1\pik{}-Er"offnung ist die
Weiterreizung
komplett analog):
\bdsc
...
oder
				konstruktive Hebung wie nach nat"urlicher
Gegenreizung
...
\ofa-Gegenreizung: Punktmajorit"at
				ohne (echten) Fit
			\item[Rest] wie oben
		\edsc
